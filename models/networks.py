...
def define_DEC(img_size, inc, watermark_size, gpu_ids):
    net = None
    net = StegaStampDecoder(img_size, inc, watermark_size)
    if len(gpu_ids) > 0:
        assert (torch.cuda.is_available())
        net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs
...
class StegaStampEncoder(nn.Module):
    def __init__(
        ...
        self.WATERMARK_UPSIZE = WATERMARK_UPSIZE
        self.return_residual = return_residual
        self.secret_dense = nn.Linear(self.watermark_size, WATERMARK_UPSIZE * WATERMARK_UPSIZE * IMAGE_CHANNELS)
        ...
        self.watermark_upsample = nn.Upsample(size=(resolution, resolution))  # scale_factor=(2**(log_resolution-4), 2**(log_resolution-4)))
    ...
    def forward(self, watermark, image):
        watermark = relu(self.secret_dense(watermark))
        watermark = watermark.view((-1, self.IMAGE_CHANNELS, self.WATERMARK_UPSIZE, self.WATERMARK_UPSIZE))
        watermark_enlarged = self.watermark_upsample(watermark)
        inputs = torch.cat([watermark_enlarged, image], dim=1)
...
class StegaStampDecoder(nn.Module):
    def __init__(self, resolution=32, IMAGE_CHANNELS=1, watermark_size=1):
