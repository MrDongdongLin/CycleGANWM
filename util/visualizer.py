...
class Visualizer():
    ...
    def print_current_bitacc(self, model, epoch, iters, this_batchsize, phase):
        ...
        ganw_A = model.netDEC(results['fake_B'])
        ganw_B = model.netDEC(results['fake_A'])
        idtw_A = model.netDEC(results['idt_A'])
        idtw_B = model.netDEC(results['idt_B'])
        recw_A = model.netDEC(results['rec_B'])
        recw_B = model.netDEC(results['rec_A'])
        ...
        if self.opt.print_msg:
            if phase == 'train':
                message = '(Train epoch: %d, iters: %d, bitacc_gan_A: %.5f, bitacc_gan_B: %.5f, bitacc_idt_A: %.5f, bitacc_idt_B: %.5f, bitacc_rec_A: %.5f, bitacc_rec_B: %.5f) ' % (
                ...
                message = '(Test epoch: %d, iters: %d, bitacc_gan_A: %.5f, bitacc_gan_B: %.5f, bitacc_idt_A: %.5f, bitacc_idt_B: %.5f, bitacc_rec_A: %.5f, bitacc_rec_B: %.5f) ' % (
                    epoch, iters, bitacc_gan_A, bitacc_gan_B, bitacc_idt_A, bitacc_idt_B, bitacc_rec_A, bitacc_rec_B)
    ...
    def save_paired_img(self, saved_path, cover_img_A, container_img, i, direction):
        for j, img in enumerate(zip(cover_img_A, container_img)):
            ...
            if direction == 'AtoB':
                ...
                SingleImgName = '%s/B_batch%04d_id%04d.png' % (saved_path, i, j)
            else:
                resultImgName = '%s/BA_batch%04d_id%04d.png' % (saved_path, i, j)
                SingleImgName = '%s/A_batch%04d_id%04d.png' % (saved_path, i, j)
            # vutils.save_image(img[1].unsqueeze(0), SingleImgName, normalize=False, padding=0)
            # vutils.save_image(img[0].unsqueeze(0), SingleImgName+'origin', normalize=False, padding=0)
