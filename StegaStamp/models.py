...
class StegaStampEncoder(nn.Module):
    def __init__(
        ...
        self.secret_dense = nn.Linear(self.fingerprint_size, WATERMARK_UPSIZE * WATERMARK_UPSIZE * IMAGE_CHANNELS)
        ...
        self.fingerprint_upsample = nn.Upsample(size=(resolution, resolution))  # scale_factor=(2**(log_resolution-4), 2**(log_resolution-4)))
    ...
    def forward(self, fingerprint, image):
        ...
        fingerprint = fingerprint.view((-1, self.IMAGE_CHANNELS, self.WATERMARK_UPSIZE, self.WATERMARK_UPSIZE))
        ...
        up6 = relu(self.up6(self.pad6(self.upsample6(conv5))))  # wahy pad add one?
        ...
        # else:
        #     residual = tanh(residual)  # since activation of GAN is tanh
        return residual
